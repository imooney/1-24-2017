...
//   WEIGHTING FOR GEANT DATA!!
  ...
  public:
    ...
    bool CountEvent( int vzbin, double weight=1 );										// Used to count PP events
    
		
    bool FillGRefMult( int gRefMult, double weight );							// For AuAu events, records gRefMult
    bool FillVz( double vz, double weight );												// Records Vz distribution
		
    bool FillAjHigh( double aj, double weight );										// Records Aj for initial hard jets
    bool FillAjLow( double aj, double weight );										// Records Aj for jets with soft constituents
		
    // For jet-hadron
    bool FillJetPt( double pt,  double weight );										// For Jet-hadron: records accepted trigger jet pt
    bool FillJetEtaPhi( double eta, double phi,  double weight );		// Records accepted trigger jet eta-phi
    ...
    // For dijet-hadron
    bool FillLeadJetPt( double pt,  double weight );								// For dijet-hadron: records lead jet pt
    bool FillSubJetPt( double pt , double weight);									// Records sub jet pt
    bool FillLeadEtaPhi( double eta, double phi, double weight );	// Records lead jet eta-phi
    bool FillSubEtaPhi( double eta, double phi, double weight );		// Records sub jet eta-phi
    ...
    bool FillAssocPt( double pt, double weight );
    bool FillAssocEtaPhi( double eta, double phi, double weight );
