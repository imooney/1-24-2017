...
int main ( int argc, const char** argv) {
  ...
  std::string corrOutFile = "ppcorr_lead20_sub10.root";						// histograms will be saved here
  std::string treeOutFile = "ppjet_lead20_sub10.root";							// jets will be saved in a TTree here
  std::string inputFile = "ppHT/picoDst*.root";			// input file: can be .root, .txt, .list
  ...
  try{
    while ( reader.NextEvent() ) {
      ...
      histograms->FillVz( vertexZ, 1);
      if ( requireDijets ) {
        histograms->FillAjHigh( corrAnalysis::CalcAj( hardJets ), 1 );
        histograms->FillAjLow( corrAnalysis::CalcAj( analysisJets ), 1 );
        histograms->FillLeadJetPt( analysisJets.at(0).pt(), 1 );
        histograms->FillLeadEtaPhi( analysisJets.at(0).eta(), analysisJets.at(0).phi_std(), 1 );
        histograms->FillSubJetPt( analysisJets.at(1).pt(), 1 );
        histograms->FillSubEtaPhi( analysisJets.at(1).eta(), analysisJets.at(1).phi_std(), 1 );
      }
      else {
        histograms->FillJetPt( analysisJets.at(0).pt() , 1);
        histograms->FillJetEtaPhi( analysisJets.at(0).eta(), analysisJets.at(0).phi_std(), 1 );
      }
